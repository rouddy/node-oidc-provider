...
function getDefaults() {
  const defaults = {
    ...
    features: {
       ...
       *
       * example: other configuration needed to reach FAPI conformance
       *
      ...
      resourceIndicators: {
         ...
         * ```
         *
         * example: Resource Server (API) with two scopes and a v1.local PASETO Access Token Format.
         * ```js
         * {
         *   scope: 'api:read api:write',
     ...
     * extraTokenClaims
     *
     * description: Function used to assign additional claims to an Access Token
     ...
     *   level claims. For `jwt` or `paseto` Access Tokens these will be top level claims.
     ...
     *   is stored using the adapter or not. The use of JWT or PASETO formats also requires
     *   use of Resource Indicators. In earlier version of oidc-provider the formats.AccessToken
     *   and formats.ClientCredentials configuration might've been used but in v7.x there's no
     ...
     *   - `paseto` tokens are issued as PASETOs. Tokens using this format are not stored using the adapter,
     *     they cannot be introspected at the introspection_endpoint and they cannot be used to access
     *     the userinfo_endpoint. Tokens issued in this format MUST have an audience/indicated resource.
     *
     * @skip
     ...
     *   configuration) will be avaialable with their names verbatim (e.g.
    ...
    enabledJWA: {
       ...
       *
       * description: JWS "alg" Algorithm values the provider supports for signed JWT Client Authentication on the introspection endpoint
       *
       * @skip
       *
       * example: Supported values list
       ...
       *
       * description: JWS "alg" Algorithm values the provider supports for signed JWT Client Authentication on the revocation endpoint
       *
       * @skip
       *
       * example: Supported values list
